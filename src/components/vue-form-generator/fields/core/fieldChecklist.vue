<template>
	<div class="wrapper" v-attributes="'wrapper'"><div class="listbox form-control" v-if="schema.listBox" :disabled="disabled"><div class="list-row" v-for="item in items" :class="{'is-checked': isItemChecked(item)}"><label><input :id="getFieldID(schema)" type="checkbox" :checked="isItemChecked(item)" :disabled="disabled" @change="onChanged($event, item)" :name="getInputName(item)" v-attributes="'input'"/>{{ getItemName(item) }}</label></div></div><div class="combobox form-control" v-if="!schema.listBox" :disabled="disabled"><div class="mainRow" @click="onExpandCombo" :class="{ expanded: comboExpanded }"><div class="info">{{ selectedCount }} selected</div><div class="arrow"></div></div><div class="dropList"><div class="list-row" v-if="comboExpanded" v-for="item in items" :class="{'is-checked': isItemChecked(item)}"><label><input :id="getFieldID(schema)" type="checkbox" :checked="isItemChecked(item)" :disabled="disabled" @change="onChanged($event, item)" :name="getInputName(item)" v-attributes="'input'"/>{{ getItemName(item) }}</label></div></div></div></div>
</template>

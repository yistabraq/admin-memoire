<template >
<div class="radio-list" :disabled="disabled" v-attributes="'wrapper'"><label v-for="item in items" :class="{'is-checked': isItemChecked(item)}" v-attributes="'label'"><input :id="getFieldID(schema)" type="radio" :disabled="disabled" :name="id" @click="onSelection(item)" :value="getItemValue(item)" :checked="isItemChecked(item)" :class="schema.fieldClasses" :required="schema.required" v-attributes="'input'"/>{{ getItemName(item) }}</label></div>
</template>

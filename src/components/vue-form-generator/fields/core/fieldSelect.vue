<template >
<select class="form-control" v-model="value" :disabled="disabled" :name="schema.inputName" :id="getFieldID(schema)" :class="schema.fieldClasses" v-attributes="'input'"><option v-if="!selectOptions.hideNoneSelectedText" :disabled="schema.required" :value="null">{{ selectOptions.noneSelectedText || "&lt;Nothing selected&gt;" }}</option><template v-for="item in items"><optgroup v-if="item.group" :label="getGroupName(item)"><option v-if="item.ops" v-for="i in item.ops" :value="getItemValue(i)">{{ getItemName(i) }}</option></optgroup><option v-if="!item.group" :value="getItemValue(item)">{{ getItemName(item) }}</option></template></select>
</template>
